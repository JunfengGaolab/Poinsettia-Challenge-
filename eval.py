...
"""
# if add new dataset, the eval.py have error, we can try delete the data/VOC2007
...
imgpath = os.path.join(args.voc_root, 'JPEGImages', '%s.jpeg')
...
def get_voc_results_file_template(image_set, cls):
    # data/multiFruits/results/det_test_aeroplane.txt
...
# first load gt
    ...
    #when add new class remember to delete the annots.pkl file
    # read list of images
...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
    ...
    output_dir = get_output_dir('ssdMultiFruit', set_type)
...
def singleEval(trained_model):
    ...
    # preTrain_dict=torch.load(trained_model, map_location=lambda storage, loc: storage)
    # net=modelLoad(net,preTrain_dict)
    net.eval()
...
def modelLoad(ssd_net,preTrain_dict):
...
if __name__ == '__main__':
    # multiModelEval()
    model=['ssd_finally.pth']
    basicFile='weights/'
    for eachModel in model:
        trained_model=basicFile+eachModel
        print('trainModel',trained_model)
        singleEval(trained_model)
